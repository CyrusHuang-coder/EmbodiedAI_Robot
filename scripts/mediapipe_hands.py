...
AXIS_LOCK_FRAMES = 8            # 锁轴后保持的帧数，越大越稳定但反应慢
MOVE_DEAD_ZONE = 0.015          # 忽略小于此值的移动（归一化坐标）
...
def classify_gesture(hand_lms, current_active):
    ...
    thumb_up = (thumb_tip.y < thumb_ip.y - 0.08 and thumb_ip.y < thumb_mcp.y - 0.05)
    # 拇指必须明显高于其它四指
    thumb_dominant = (thumb_tip.y < hand_lms[8].y - 0.10 and thumb_tip.y < hand_lms[12].y - 0.10)
    if ( thumb_up and thumb_dominant and index_bent and middle_bent and ring_bent and pinky_bent):
        return "thumbs_up"
...
try:
    while running and cap.isOpened():
        ...
        if result.hand_landmarks:
            ...
            if gesture_count >= 5:
                # 点赞：切换 AB 轴锁定状态
                if det_gesture == "thumbs_up" and current_time - last_lock_time > 1.0:
            ...
            if active_gesture == "point" and not ab_locked:
                ...
                if control_axis == "A":
                    ...
                    current_base_angle = clamp(current_base_angle, 0, 180)
                ...
                # B 轴（肩）：手指垂直偏移转换成速度累加
                    ...
                    offset_y = 0.5 - py
                    if abs(offset_y) < 0.03:
                        offset_y = 0
                    speed = offset_y * 40          # 【可调参数】速度系数 40
                    current_shoulder_angle += speed
                    current_shoulder_angle = clamp(current_shoulder_angle, 0, 180)
            ...
            # ---------- Pinch 模式：捏合控制夹爪 ----------
                ...
                grip_angle = clamp(grip_angle, 0, 180)
